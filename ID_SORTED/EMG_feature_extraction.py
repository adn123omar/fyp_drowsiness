...
# features_estimation(signal, channel_name, fs, frame, step, plot=False)
...
for path in os.listdir(dir_path):
    if os.path.isfile(os.path.join(dir_path, path)):
        ...
        emg_data = data[:,1]
        n_seconds_valid = np.floor(len(emg_data)/fs)
        ...
        total_feature_matrix, features_names = features_estimation(ecg_data_clipped, channel_name, fs, wind_n_elements, step_size*fs)
        ...
        # for j in range(np.shape(total_feature_matrix)[0]):
        #     df.loc[len(df)] = total_feature_matrix[j,:]

        # df.to_csv(dest_file_dir[0:-13]+"_emg_features_filtered.csv")  
